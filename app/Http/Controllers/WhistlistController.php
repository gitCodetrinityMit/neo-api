...
class WhistlistController extends Controller
{
    ...
    public function index() {
        if(Auth::check()){
            $wishlist = Wishlist::with('products','product_galleries')->where('user_id','=',auth()->user()->id)->get();   
    ...
    {
        if(Auth::check()){
            $remove_wishlist_product = Wishlist::with('products','product_galleries')
            ->where('user_id',auth()->user()->id)->where('product_id',$request->product_id)->first();

        $remove_wishlist_product->delete(); 
